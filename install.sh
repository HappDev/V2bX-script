...
if [ "$(getconf WORD_BIT)" != '32' ] && [ "$(getconf LONG_BIT)" != '64' ] ; then
    echo "This software does not support 32-bit systems (x86). Please use a 64-bit system (x86_64). If this detection is incorrect, contact the author."
    exit 2
fi
...
install_V2bX() {
    ...
    if  [ $# == 0 ] ;then
        ...
        if [[ ! -n "$last_version" ]]; then
            echo -e "${red}Failed to detect V2bX version. Possibly exceeded Github API limit. Try again later or manually specify a version.${plain}"
            exit 1
        fi
        echo -e "Latest V2bX version detected: ${last_version}, starting installation"
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Download failed, please make sure your server can access Github${plain}"
            exit 1
        fi
    else
        last_version=$1
        ...
        if [[ $? -ne 0 ]]; then
            echo -e "${red}Download of V2bX $1 failed. Please ensure this version exists.${plain}"
            exit 1
        fi
    fi
...
EOF
        ...
        echo -e "${green}V2bX ${last_version}${plain} installation completed and set to start on boot"
    else
...
EOF
    ...
    if [[ ! -f /etc/V2bX/config.json ]]; then
        ...
        echo -e ""
        echo -e "Fresh installation. Please read the guide first: https://v2bx.v-50.me/ and configure the necessary settings."
        first_install=true
    else
        if [[ x"${release}" == x"alpine" ]]; then
            service V2bX start
        ...
        if [[ $? == 0 ]]; then
            ...
            echo -e "${red}V2bX might have failed to start. Use 'V2bX log' to check logs. If it cannot start, the config format may have changed. See wiki: https://github.com/V2bX-project/V2bX/wiki${plain}"
        fi
        first_install=false
    fi
    ...
    rm -f install.sh
    echo -e ""
    echo "V2bX Management Script Usage (You can also use 'V2bX', case-insensitive): "
    echo "------------------------------------------"
    ...
    echo "V2bX enable       - Enable autostart"
    echo "V2bX disable      - Disable autostart"
